...
%% @type request() = term()
% Any request object from the underlying web framework.
...
%% @type guardfun() = function() -> boolean()
%%                  | {Mod::atom(), Fun::atom()}
...
try_path_binding([PathSpec|Rest], PathTokens, HostRemainder, Port, HostBindings, ExtraDepth, RD) ->
    ...
    case bind(PathSchema, PathTokens, HostBindings, 0) of
        {ok, Remainder, NewBindings, Depth} ->
            ...
            ED = #espatch_data{path_info=PathInfo, host_remainder=HostRemainder,
                               port=Port, remainder=Remainder, app_root=AppRoot,
                               string_path=StringPath},
            case run_guard(Guard, RD, ED) of
                true ->
...
run_guard(undefined, _RD, _ED) ->
    true;
run_guard(Fun, RD, ED) when is_function(Fun) ->
    try
        Fun(RD, ED) == true
    catch _Type : Msg ->
...
dispatch_test() ->
    RD = testing,
    TrueFun = fun(_, _) -> true end,
    FalseFun = fun(_, _) -> false end,
